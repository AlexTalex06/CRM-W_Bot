...
function App() {
  ...
  const handleStartNewChat = async () => {
    if (!newChatPhone.trim() || !newChatMsg.trim()) return;
    setIsStartingChat(true);
    try {
      await sendMessage(newChatPhone.trim(), newChatMsg.trim());
      setShowNewChatModal(false);
      setNewChatPhone('');
      setNewChatMsg('');
      setSelectedConvId(newChatPhone.trim());
    } catch (err) {
      console.error(err);
      alert("❌ Error al iniciar chat. Verifica que el número telefónico sea correcto y tenga el código de país (ej. 521...).");
    } finally {
        ...
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h3 className="font-bold text-lg text-slate-800">Iniciar Nuevo Chat</h3>
              <button 
            ...
            <div className="p-6 space-y-5">
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Cód. País + Número (ej: 521...)</label>
                <input 
                  type="text" 
                  className="w-full bg-gray-50 border border-gray-200 focus:border-brand-whatsapp focus:ring-1 focus:ring-brand-whatsapp rounded-xl px-4 py-3 text-sm outline-none transition-all" 
                  value={newChatPhone} 
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Primer Mensaje</label>
                <textarea 
                  className="w-full bg-gray-50 border border-gray-200 focus:border-brand-whatsapp focus:ring-1 focus:ring-brand-whatsapp rounded-xl px-4 py-3 text-sm outline-none transition-all resize-none h-28" 
                  value={newChatMsg} 
                  onChange={e => setNewChatMsg(e.target.value)} 
                  placeholder="¡Hola! Te contacto de..." 
                />
              </div>
            </div>
            <div className="p-5 bg-gray-50/50 border-t border-gray-100 flex justify-end space-x-3">
              <button 
                onClick={() => setShowNewChatModal(false)}
                className="px-5 py-2.5 text-sm font-semibold text-slate-600 hover:bg-gray-200 rounded-xl transition-colors cursor-pointer"
              >
                Cancelar
              </button>
              <button 
                onClick={handleStartNewChat} 
                disabled={isStartingChat || !newChatPhone || !newChatMsg} 
                className="px-5 py-2.5 text-sm font-semibold text-white bg-brand-whatsapp hover:bg-green-600 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-colors cursor-pointer flex items-center"
              >
                {isStartingChat ? (
                  <>
